...
function itemMeta(kind: InventoryResourceKind, item: InventoryItem) {
  if (kind === 'stores') {
    ...
    return `${defaultWarehouse?.name ?? 'Sin almacén principal'} · ${count} ${count === 1 ? 'almacén' : 'almacenes'}`;
  }

  if (kind === 'warehouses') {
    const warehouse = item as Warehouse;
    return `${warehouse.store?.name ?? 'Sin tienda'}${warehouse.is_default ? ' · Principal' : ''}`;
  }

...
export function PosSupplyRequestList() {
  ...
  const loadItems = useCallback(async (refresh = false) => {
    ...
    try {
      const response = await api.get('/inventory-transfers', { params: { status: 'draft' } });
      const drafts = (response.data.data ?? []) as InventoryTransfer[];
      setItems(drafts.filter((transfer) => transfer.pos_order_id !== null));
      setError('');
    } catch (requestError) {
  ...
  async function resolve(transfer: InventoryTransfer) {
    ...
    setNotice('');
    try {
      await api.post(`/inventory-transfers/${transfer.id}/resolve`);
      setItems((current) => current.filter((item) => item.id !== transfer.id));
      setNotice(`Comanda de la orden #${transfer.pos_order_id} lista, stock repuesto.`);
    } catch (requestError) {
        ...
        renderItem={(transfer) => (
          <View style={styles.card}>
            <View style={styles.cardHeader}>
              ...
              <Text style={styles.cardTitle}>Orden #{transfer.pos_order_id}</Text>

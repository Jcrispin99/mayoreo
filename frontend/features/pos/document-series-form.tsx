...
import type { FiscalIssuer } from '../settings/settings-types';
...
export function DocumentSeriesForm({ documentSeriesId }: Props) {
  ...
  const locked = editing && identityLocked;

  useEffect(() => {
    async function load() {
      try {
        const [issuerResponse, seriesResponse] = await Promise.all([
          api.get('/fiscal-issuers?is_active=1'),
          documentSeriesId ? api.get(`/document-series/${documentSeriesId}`) : Promise.resolve(null),
        ]);
        const availableIssuers: FiscalIssuer[] = issuerResponse.data.data ?? [];
        setIssuers(availableIssuers);
        if (seriesResponse) {
          const series: DocumentSeries = seriesResponse.data.data;
          setIssuerId(series.fiscal_issuer_id);
          ...
          setIdentityLocked(series.current_number > 0 || series.assigned_cash_register_id !== null);
        } else {
          setIssuerId(availableIssuers[0]?.id ?? null);
        }
      } catch (requestError) {
  ...
  async function save() {
    if (!issuerId) { setError('Selecciona el emisor fiscal.'); return; }
    if (!code.trim()) { setError('Ingresa el código de la serie.'); return; }
    setSaving(true);
    setError('');
    const payload = { fiscal_issuer_id: issuerId, document_type: type, series_code: code.trim().toUpperCase(), current_number: Number(currentNumber), is_active: active };
    try {
        ...
        {loading ? <ActivityIndicator color="#B4232D" size="large" style={styles.loader} /> : (
          <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
            ...
            <View style={styles.card}>
              <Menu
                anchor={(
                  <Pressable disabled={locked} onPress={() => setIssuerMenuVisible(true)} style={[styles.selector, locked && styles.disabled]}>
                    <View>
                      <Text style={styles.label}>Emisor fiscal *</Text>
                      <Text style={styles.value}>{issuers.find((issuer) => issuer.id === issuerId)?.legal_name ?? 'Seleccionar emisor'}</Text>
                    </View>
              ...
              >
                {issuers.map((issuer) => (
                  <Menu.Item
                    key={issuer.id}
                    onPress={() => { setIssuerId(issuer.id); setIssuerMenuVisible(false); }}
                    title={`${issuer.ruc} · ${issuer.legal_name}`}
                  />
                ))}
              </Menu>
              <Menu

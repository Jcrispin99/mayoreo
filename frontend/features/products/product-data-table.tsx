...
function ProductDetailCell({ product }: { product: ProductListItem }) {
  const prices = product.variants
    ...
    .slice(0, 3)
    .map((variant) => (
      `${variant.variant_name || variant.sku}: S/ ${formatMoney(variant.price ?? 0)}/${variant.priceUnit ?? 'un.'}`
    ));
  const pricesRemaining = product.variants.filter((variant) => variant.price !== null).length - prices.length;
  const variants = product.variants.slice(0, 3).map((variant) => {
    const unit = variant.base_unit?.code ?? 'un.';
    return `${variant.variant_name || variant.sku}: ${formatQuantity(variant.quantity)} ${unit}`;
  });
  const remaining = product.variants.length - variants.length;
  ...
  return (
    <View>
      ...
      <Text numberOfLines={2} style={styles.detail}>
        Precios: {prices.length > 0 ? prices.join(' · ') : 'Sin configurar'}
        {pricesRemaining > 0 ? ` · +${pricesRemaining}` : ''}
      </Text>
      <Text numberOfLines={2} style={styles.detail}>
        {variants.join(' · ')}{remaining > 0 ? ` · +${remaining}` : ''}
      </Text>
    </View>
  );
}
